    ...
    return (
        ...
        >
            <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:h-20 sm:px-6 lg:px-10">
                <Link
                    href="/"
                    className="shrink-0 whitespace-nowrap text-base font-semibold tracking-[0.22em] text-[#F5F5F2] uppercase sm:text-lg sm:tracking-[0.32em]"
                ...
                <div className="hidden items-center gap-6 xl:gap-8 lg:flex">
                    {navigationLinks.map((link) => (
                        ...
                        >
                            {link.label}
                            <span className="absolute -bottom-2 left-0 h-px w-0 bg-[#B89B72] transition-all duration-300 group-hover:w-full" />
                        </Link>
                    ))}
                </div>
                ...
                <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
                    <button
                        type="button"
                        className="grid size-9 place-items-center text-[#F5F5F2] transition hover:text-[#B89B72] sm:size-10"
                        aria-label="Search"
                    >
                    ...
                    <button
                        type="button"
                        className="grid size-9 place-items-center text-[#F5F5F2] transition hover:text-[#B89B72] sm:size-10 lg:hidden"
                    ...
                    >
                        <div className="mx-auto flex max-w-7xl flex-col pt-4">
                            {navigationLinks.map((link, index) => (
                                ...
                                >
                                    <Link
                                        ...
                                        className="flex border-b border-white/10 py-4 text-sm font-medium tracking-[0.18em] text-[#F5F5F2] uppercase transition hover:text-[#B89B72] sm:py-5 sm:tracking-[0.22em]"
                                    >
                                        {link.label}
                                    </Link>
                                </motion.div>
                            ...
                            <Link
                                href="/contact"
                                onClick={() => setIsMenuOpen(false)}
                                className="mt-6 inline-flex max-w-full justify-center border border-[#B89B72]/70 px-4 py-4 text-center text-xs font-semibold tracking-[0.16em] text-[#F5F5F2] uppercase transition hover:bg-[#F5F5F2] hover:text-[#0A0A0A] sm:px-5 sm:tracking-[0.22em]"
                            >
                                Start a Project
                            </Link>
                        </div>
